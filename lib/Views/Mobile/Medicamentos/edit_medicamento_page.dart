...
class EditMedicamentoPage extends StatefulWidget {
  const EditMedicamentoPage({super.key, required this.medicamento});
...
class _EditMedicamentoPageState extends State<EditMedicamentoPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.medicamento.isClinica == true) {
        _showSnackBar(
          "Não é possivel editar pois a medicamento foi adicionada pela clínica",
    ...
    return DropdownButtonFormField<String>(
      ...
      onChanged: (val) {
        setState(() {
          ...
          // If frequency changed away from 'X vezes ao dia', clear times
          if (previous == 'X vezes ao dia' &&
              _frequencyController.text != previous) {
            _clearReminderTimes();
            _timesPerDayController.clear();
          }
          // If switched to 'X vezes ao dia' and there is a number, build controllers
    ...
    if (_frequencyController.text != 'X vezes ao dia') {
      ...
      defaultSection.add(
        _buildReadOnlyField(
          ...
          onTap: _selectTime,
          validator: (v) =>
              v == null || v.isEmpty ? 'Digite o horário da medicamento' : null,
        ),
      );
    }
  ...
  Future<void> _onSavePressed() async {
    ...
    try {
      ...
      final targetUserId = _userController.currentUser?.associetedId;
      await _viewmodel.editMedicamento(medicamento, userId: targetUserId);

      if (_isEstoqueExpanded) {
        final estoque = Estoque(
          id: _existingEstoque?.id ?? Uuid().v1(),
        ...
        if (_existingEstoque != null) {
          await _estoqueViewModel.editEstoque(estoque, userId: targetUserId);
        } else {
          await _estoqueViewModel.addEstoque(estoque, userId: targetUserId);
        }
      } else if (_existingEstoque != null) {
        // Se o painel foi fechado e havia estoque, deletar
        final targetUserId = _userController.currentUser?.associetedId;
        await _estoqueViewModel.deleteEstoque(
          _existingEstoque!.id,
          userId: targetUserId,
        );
      }
      ...
      if (context.mounted) {
        ...
        _showSnackBar("Medicamento atualizado", color: Colors.green);
      }
    } catch (e) {
      _showSnackBar("Falha ao adicionar medicamento: $e", color: Colors.red);
    }
  }
  ...
  Future<void> _loadEstoque() async {
    final targetUserId = _userController.currentUser?.associetedId;
    _estoqueViewModel.loadEstoques(userId: targetUserId);

    // Aguardar um pouco para o stream carregar
    await Future.delayed(const Duration(milliseconds: 500));

    final estoquesDoMedicamento = _estoqueViewModel.estoques
        .where((e) => e.medicamento == widget.medicamento.id)
        .toList();

    if (estoquesDoMedicamento.isNotEmpty) {
      _existingEstoque = estoquesDoMedicamento.first;
      _estoqueQuantityController.text = _existingEstoque!.quantity.toString();
      _estoqueMinimalQuantityController.text = _existingEstoque!.minimalQuantity
          .toString();
      setState(() {
        _isEstoqueExpanded = true;
      });
    }
  }

  void _setTextControllers() {
    final dt = widget.medicamento.startDate;

    _nameController.text = widget.medicamento.name;
    _quantityController.text = widget.medicamento.quantity.toString();
    _unitController.text = widget.medicamento.unit;
    _frequencyController.text = widget.medicamento.frequency;
    if (widget.medicamento.intervalHours != null) {
      _intervalController.text = widget.medicamento.intervalHours.toString();
    }
    if (widget.medicamento.timesPerDay != null) {
      _timesPerDayController.text = widget.medicamento.timesPerDay.toString();
      _ensureReminderControllers(widget.medicamento.timesPerDay!);
      for (var i = 0; i < _reminderControllers.length; i++) {
        if (i < widget.medicamento.reminderTimes.length) {
          final rt = widget.medicamento.reminderTimes[i];
          _reminderControllers[i].text = DateFormat('HH:mm').format(rt);
          _reminderTimeOfDays[i] = TimeOfDay(hour: rt.hour, minute: rt.minute);
        }
      }
    } else {
      _ensureReminderControllers(0);
    }
    if (widget.medicamento.weekDays != null) {
      _weekDaysController.text = widget.medicamento.weekDays!.join(',');
    }
    if (widget.medicamento.notes != null) {
      _noteController.text = widget.medicamento.notes!;
    }

    _startDateController.text = DateFormat("dd/MM/yyyy").format(dt);
    _reminderTimeController.text = DateFormat("HH:mm").format(dt);
